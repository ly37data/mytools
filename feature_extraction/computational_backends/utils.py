...
from feature_extraction.feature_base import AggregationFeature
from feature_extraction.utils import Trie
from feature_extraction.utils.wrangle import _check_timedelta
...
def create_client_and_cluster(n_jobs, dask_kwargs, entityset_size):
    ...
    if 'cluster' in dask_kwargs:
        ...
        # of cores available to the workers (alwasy 1 for feature_extraction setup).
        # This means reducing the number of workers does not increase the memory
        # limit for other workers.  feature_extraction default is to calculate memory limit
    ...
    for worker in list(client.scheduler_info()['workers'].values()):
        ...
        if worker_limit < entityset_size:
            ...
            logger.warn("Worker memory is between 1 to 2 times the memory"
                        ...
                        "cause.  See https://docs.feature_extraction.com/guides/parallel.html"
