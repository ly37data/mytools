...
import inspect
import numpy as np
from feature_extraction.primitives.base.primitive_base import PrimitiveBase
from feature_extraction.primitives.base.utils import inspect_function_args


class AggregationPrimitive(PrimitiveBase):
    
    def __init__(self,default_value=np.nan,missing_value=None,handling='ignore'):
        super(AggregationPrimitive,self).__init__(default_value,missing_value)
        self.handling = handling
        
    def generate_name(self, base_feature_names, where_str,time_window_str,cn=False):
        base_features_str = ", ".join(base_feature_names)
        name = '_'+self.name if self.name != "" else ""
        cn_name = '_'+self.cn_name if self.cn_name !="" else ""
        return "{}{}{}{}".format(            
            where_str,
            time_window_str,
            base_features_str,
            cn_name if cn else name ,
            
        )

    def generate_names(self, base_feature_names,
                        where_str,time_window_str,cn=False):
        n = self.number_output_name
        base_features_str = ", ".join(base_feature_names)
        return ["{}{}{}_{}_{}".format(            
            where_str,
            time_window_str,
            base_features_str,
            i,
            self.cn_name if cn else self.name ,
...
def make_agg_primitive(function, input_types, return_type, name=None,
    ...
    Example:
        .. ipython :: python
            ...
            from feature_extraction.primitives import make_agg_primitive
            from feature_extraction.variable_types import DatetimeTimeIndex, Numeric
