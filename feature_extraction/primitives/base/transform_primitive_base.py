...
class TransformPrimitive(PrimitiveBase):
    ...
    def __init__(self,default_value=np.nan,missing_value=None):
        super(TransformPrimitive,self).__init__(default_value,missing_value)

    def generate_name(self, base_feature_names,cn=False,is_return=False):
        if cn:
            name = "" if is_return else '_'+self.cn_name
        else:
            name = "" if is_return else '_'+self.name
        return "%s%s" % (
...
def make_trans_primitive(function, input_types, return_type, name=None,
    ...
    Example:
        .. ipython :: python
            ...
            from feature_extraction.primitives import make_trans_primitive
            from feature_extraction.variable_types import Variable, Boolean
