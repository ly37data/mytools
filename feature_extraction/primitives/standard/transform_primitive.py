...
from feature_extraction.primitives.base.transform_primitive_base import (
    TransformPrimitive
)
...
from feature_extraction.utils.time_utils import series_parse_date
from pandas.api.types import is_datetime64_any_dtype

class PassThroughFloat(TransformPrimitive):
    ...
    """
    name = 'passthroughfloat'
    cn_name = '转数值'
    input_types = [Variable]
    return_type = Numeric
    
    def __init__(self,divide=1,default=np.nan,missing_value=[np.nan,""],default_dirty=None,rexdig=False):
        super(PassThroughFloat,self).__init__(default,missing_value)
        self.default_dirty = [] if default_dirty is None else default_dirty
        self.redig = re.compile('\d+') 
        self.rexdig = rexdig
        self.divide = divide
        if self.divide == 0:
    ...
    def _fill_na(self,array):
        ...
        if self.missing_value:
            for value in self.missing_value:
                array=array.replace(value,self.default_value)
        if self.default_dirty:
            for dirty in self.default_dirty:
                array = array.str.replace(dirty,"")
        if self.rexdig:
            rexdgfunc = np.vectorize(lambda x: self.redig.search(str(x))[0] if self.redig.search(str(x)) else np.nan)
            array = rexdgfunc(array)
        
        return array.astype(np.float)/self.divide
    
    def get_function(self):
        def passfloat(array):
            return self._fill_na(array)
        return passfloat

class PassThroughStr(TransformPrimitive):
    """transform value to float
    ...
    def __init__(self,default='NotAvailable',missing_value=None):
        super(PassThroughStr,self).__init__(default,missing_value)
...
class CalcNormalTimeInterval(TransformPrimitive):
    """Calculates timeinterval.
    ...
    """
    name = 'clactimeinterval'
    cn_name = '时间差'
    input_types = [[Variable, Variable]]
    return_type = Numeric
#    uses_calc_time = True

    def __init__(self, unit="days",default=np.nan,missing_value=['1900'],only_month=False,only_year=False):
        if only_month :
            self.unit = 'months'
        elif only_year:
            self.unit = 'years'
    ...
    def get_function(self):
#        def pd_time_since(array1, array2): 
#            if self.missing_value:
#                for i in self. missing_value:
#                    array1 = array1.replace(i,np.nan)
#                    array2 = array2.replace(i,np.nan)
#            va = convert_time_units((pd.to_datetime(array1,errors='coerce') - pd.to_datetime(array2,errors='coerce')
#                    ).values, self.unit).fillna(2147483648).values.astype(int)
#            return pd.Series(va).replace(2147483648,self.default_value)
        def pd_time_since(array1,array2):
            array1 = self._fill_na(array1)
            array2 = self._fill_na(array2)
            
            if self.unit == 'years':
                va = array1.dt.year - array2.dt.year
            elif self.unit == 'months':
                va = (array1.dt.year - array2.dt.year)*12 + (
                       array1.dt.month - array2.dt.month )

                #计算方式2
#                va = (((pd.to_datetime(array1,errors='coerce') - pd.to_datetime(array2,errors='coerce')).dt.days)/30).round(0)
            elif self.unit == 'days':
                va = (array1- array2).dt.days
            else:
