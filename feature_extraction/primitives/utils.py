...
from feature_extraction.utils.gen_utils import find_descendents


def get_aggregation_primitives():
    aggregation_primitives = set([])
    for attribute_string in dir(feature_extraction.primitives):
        attribute = getattr(feature_extraction.primitives, attribute_string)
        if isclass(attribute):
            if issubclass(attribute,
                          feature_extraction.primitives.AggregationPrimitive):
                if attribute.name:
...
def get_transform_primitives():
    ...
    for attribute_string in dir(feature_extraction.primitives):
        ...
        if isclass(attribute):
            if issubclass(attribute,
                          feature_extraction.primitives.TransformPrimitive):
                if attribute.name:
