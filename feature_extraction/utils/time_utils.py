...
def convert_time_units(secs,
    ...
    return pd.Series(secs / (unit_divs[unit]))


def parse_date(date_str,dayfirst=False):
    if date_str in ('00000000','19000101','190001','1900','0000','0000.00.00'
                    ,'0000.00','0000.01.01','1900.01'):
        return None
    date_str = date_str.split('.')[0] if len(date_str)>20 else date_str.replace('.','-')
    
    try:
        dt = parse(date_str,fuzzy=True,dayfirst=dayfirst)
        if dt and dt > datetime(1900,1,1):
            return dt
        else:
            return None
    except ValueError:
...
def series_parse_date(series,dayfirst=False):
    func = np.vectorize(parse_date)
    res = pd.Series(func(series.values,dayfirst)) 
    return res
