...
from feature_extraction.entityset.timedelta import Timedelta
...
def _check_timedelta(td):
    ...
    If a pd.Timedelta object is passed, units will be converted to seconds due to the underlying representation
        of pd.Timedelta.
    If a pd.DateOffset object is passed, it will be converted to a feature_extraction Timedelta if it has one
...
def handle_missing_value(series,missing_value,handling='ignore'):
    for value in missing_value:
        series = series.replace(value,np.nan)
    if handling == 'ignore':
        series = series.dropna()     
    else:
        series_notnull = series.dropna()
        if handling == 'as_mean':
            series = series.fillna(series_notnull.mean())
        elif handling == 'as_median':
            series = series.fillna(series_notnull.median())
            
    return series


def save_df(df:pd.DataFrame,result:str):
    
    if r'.csv' in result:
        df.to_csv(result,encoding='utf-8')
    elif r'.tsv' in result:
        df.to_csv(result,encoding='utf-8',sep='\t')
    elif r'.pkl' in result:
        df.to_pickle(result)
